...
namespace Oberon0.Compiler.Definitions
{
    ...
    public partial class Module
    {
        ...
        private void AddParameters(Oberon0ExportAttribute attr, int i, IList<ProcedureParameterDeclaration> procParameters)
        {
        ...
        /// </remarks>
        public static ProcedureParameterDeclaration GetProcedureParameterByName([NotNull] string parameterName, [NotNull] string parameterType, [NotNull] Block block)
        {
            if (parameterName == null) throw new ArgumentNullException(nameof(parameterName));
            if (parameterType == null) throw new ArgumentNullException(nameof(parameterType));
            if (block == null) throw new ArgumentNullException(nameof(block));

            var (targetType, isVar) = GetParameterDeclarationFromString(parameterType, block);
        ...
        /// <returns></returns>
        internal static (TypeDefinition, bool) GetParameterDeclarationFromString([NotNull] string typeString, [NotNull] Block block)
        {
            if (typeString == null) throw new ArgumentNullException(nameof(typeString));
            if (block == null) throw new ArgumentNullException(nameof(block));
            
            var matches = Regex.Match(
                typeString,
                "(?<ref>&|VAR\\s+)?(?<name>[A-Za-z][A-Za-z$0-9]*)(?<isarray>\\[(?<size>\\d+)\\])?");
            if (!matches.Success) throw new ArgumentException($"{typeString} is not a valid type reference", nameof(typeString));

            string typeName = matches.Groups["name"].Value;
            bool isVar = matches.Groups["ref"].Success;

            var type = block.LookupType(typeName);
            if (type == null) throw new ArgumentException($"{typeString} is not a valid type reference", nameof(typeString));
        ...
        private void DeclareStandardConstants()
        {
            ...
            Block.Declarations.Add(
                new ConstDeclaration("EPSILON", Block.LookupType("REAL"), new ConstantDoubleExpression(double.Epsilon)));
        }
            ...
            [NotNull] string className, [NotNull] string methodName)
        {
            ...
            if (rt == null) throw new InvalidOperationException($"Return type {attr.ReturnType} not defined");

            var procParameters = new ProcedureParameterDeclaration[attr.Parameters.Length];
            for (int i = 0; i < attr.Parameters.Length; i++) AddParameters(attr, i, procParameters);
