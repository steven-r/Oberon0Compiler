    ...
    internal class Oberon0CompilerListener : OberonGrammarBaseListener
    {
        ...
        public override void ExitSingleVariableDeclaration(OberonGrammarParser.SingleVariableDeclarationContext context)
        {
            foreach (var token in context._v)
            {
                ...
                {
                    _parser.NotifyErrorListeners(token, "Variable declared twice", null);
                }
                else
                {
        ...
        public override void ExitConstDeclarationElement(OberonGrammarParser.ConstDeclarationElementContext context)
        {
            ...
            {
                _parser.NotifyErrorListeners(context.c, "A variable/constant with this name has been defined already",
                    null);
                return;
            }
            ConstantExpression constexp = ConstantSolver.Solve(context.e.expReturn, context.bParam) as ConstantExpression;
            if (constexp == null)
            {
                _parser.NotifyErrorListeners(context.e.start, "A constant must resolve during compile time", null);
                return;
            }
            context.bParam.Declarations.Add(new ConstDeclaration(context.c.Text,
                constexp.TargetType, constexp, context.bParam));
        }

        public override void ExitArrayType(OberonGrammarParser.ArrayTypeContext context)
        {
            ConstantIntExpression constexp = ConstantSolver.Solve(context.e.expReturn, context.bParam) as ConstantIntExpression;
            if (constexp == null)
            {
                _parser.NotifyErrorListeners(context.Start, "The array size must return a constant integer expression",
                    null);
        ...
        {
            context.param = new ProcedureParameter(context.name.Text, context.bParam, context.t.returnType, context.isVar);
        }
        ...
        {
            var fp = context.bParam.LookupFunction(context.id.Text,  context.cp?._p?.Select(x => x.expReturn)?.ToList() ?? new List<Expression>());
            if (fp == null)
            {
                _parser.NotifyErrorListeners(context.id, "Function not known", null);
                return;
            }
            context.expReturn = new FunctionCallExpression(fp, context.bParam,
        ...
        public override void ExitSelector(OberonGrammarParser.SelectorContext context)
        {
            ...
            {
                _parser.NotifyErrorListeners(context.start, "Simple variables do not allow any selector", null);
                return;
            }
            ...
            foreach (var v in vs)
            {
                ...
                {
                    type = CheckArrayIndexSelector((IndexSelector)v, type, context);
                }
            }
        ...
        private TypeDefinition CheckArrayIndexSelector(IndexSelector indexSelector, TypeDefinition type, OberonGrammarParser.SelectorContext context)
        {
            ...
            if (indexSelector.IndexDefinition.IsConst)
            {
                ...
                {
                    _parser.NotifyErrorListeners(indexSelector.Token, "Array index of out bounds", null);
                    return null;
                }
            }
        ...
        {
            context.expReturn = BinaryExpression.Create(OberonGrammarLexer.MINUS, context.t.expReturn, null, context.bParam);
        }
        ...
        public override void ExitFactorExpression(OberonGrammarParser.FactorExpressionContext context)
        {
            ...
            {
                context.expReturn = BinaryExpression.Create(context._op[i - 1].Type, context.expReturn,
                    context._s[i].expReturn, context.bParam);
            }
        }
        ...
        public override void ExitSimpleExpression(OberonGrammarParser.SimpleExpressionContext context)
        {
            ...
            {
                context.expReturn = BinaryExpression.Create(context._op[i - 1].Type, context.expReturn,
                    context._f[i].expReturn, context.bParam);
            }
        }
        ...
        public override void ExitProcCall_statement(OberonGrammarParser.ProcCall_statementContext context)
        {
            ...
            {
                _parser.NotifyErrorListeners(context.id, $"Procedure {fp.Name} not known or a function with this name is called as a procedure", null);
                return;
            }
            ...
            {
                _parser.NotifyErrorListeners(context.id, $"Number of parameters expected: {procParameters.Length}", null);
                return;
            }
        ...
        public override void ExitRepeat_statement(OberonGrammarParser.Repeat_statementContext context)
        {
            ...
            {
                _parser.NotifyErrorListeners(context.r.start, $"The condition needs to return a logical condition", null);
                return;
            }
        ...
        public override void ExitIf_statement(OberonGrammarParser.If_statementContext context)
        {
            foreach (OberonGrammarParser.RelationalExpressionContext expressionContext in context._c)
            {
                ...
                {
                    _parser.NotifyErrorListeners(expressionContext.start, "The condition needs to return a logical condition", null);
                    return;
                }
        ...
        public override void ExitAssign_statement(OberonGrammarParser.Assign_statementContext context)
        {
            ...
            context.container.Statements.Add(new AssignmentStatement
            {
                Variable = v,
                Selector = context.s.vsRet,
                Expression = e
            });
        }
